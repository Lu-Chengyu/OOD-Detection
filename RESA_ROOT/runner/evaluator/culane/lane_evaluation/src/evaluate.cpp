...
int main(int argc, char **argv) {
  ...
  size_t lastSlashIndex = list_im_file.rfind('/'); // gai
  size_t lastUnderscoreIndex = list_im_file.rfind('_');
  string FPType = list_im_file.substr(lastUnderscoreIndex + 1, lastSlashIndex - lastUnderscoreIndex - 1);
  size_t lastSlashPos = detect_dir.find_last_of("/\\");
  string parent_path = detect_dir.substr(0, lastSlashPos);
  size_t secondLastSlashPos = parent_path.find_last_of("/\\");
  parent_path = parent_path.substr(0, secondLastSlashPos + 1);
    
  size_t lastSlashPoint = FPType.find_last_of(".");
  string FPTypeWOtxt = FPType.substr(0,lastSlashPoint);
  string save_path1 = parent_path + FPTypeWOtxt;
    
  ofstream outputFile;
  // outputFile.open("FP" + FPType);
  // outputFile.open(parent_path + "FP" + FPType);
  // std::cout << parent_path << endl;
  // std::cout << save_path1 << endl;
  // std::cout << FPType << endl;
  // std::cout << save_path1 + "/FP" + FPType << endl;
  outputFile.open(save_path1 + "/FP" + FPType); // gai

    
  #pragma omp parallel for // gai
  for (size_t i = 0; i < filelists.size(); i++) {
    ...
    long tempFP = get<2>(tuple_lists[i]); // gai
    if(tempFP > 0){
      outputFile << full_im_name << std::endl;
      auto anno_match = get<0>(tuple_lists[i]);
      visualize(full_im_name, anno_lanes, detect_lanes, anno_match, width_lane, save_path1);
    } // gai
...
void visualize(string &full_im_name, vector<vector<Point2f>> &anno_lanes,
  ...
  if (save_path != "") {
    ...
    // cout << file_name << endl;
    imwrite(save_path + '/' + file_name, img2);
  } else {
