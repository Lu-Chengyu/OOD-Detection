...
def call_culane_eval(data_dir, output_path='./output'):
    ...
    os.mkdir(os.path.join(output_path,'normal'))
    os.mkdir(os.path.join(output_path,'crowd'))
    os.mkdir(os.path.join(output_path,'hlight'))
    os.mkdir(os.path.join(output_path,'shadow'))
    os.mkdir(os.path.join(output_path,'noline'))
    os.mkdir(os.path.join(output_path,'arrow'))
    os.mkdir(os.path.join(output_path,'curve'))
    os.mkdir(os.path.join(output_path,'cross'))
    os.mkdir(os.path.join(output_path,'night'))
...
class CULane(nn.Module):
    ...
    def evaluate(self, dataset, output, batch):
        seg, exists, laneFeature = output['seg'], output['exist'], output['lane_feature'] # gai
        ...
        for i in range(batch_size):
            ...
            for coord in coords:
                ...
                f.write('\n')
            f.close()
            # oneFeature = laneFeature[i].view(-1, laneFeature[i].numel()) # gai
            featureOutdir = os.path.join(self.cfg.work_dir, 'TestFeature')
            featureOutname = featureOutdir + img_name[i][:-4] + '.txt'
            featureoutdir = os.path.dirname(featureOutname)
            if not os.path.exists(featureoutdir):
                os.makedirs(featureoutdir)
            np.savetxt(featureOutname, laneFeature[i].flatten()) # gai
    ...
    def pcaTrain(self, output, ipca):
        seg, laneFeature = output['seg'], output['lane_feature'] # gai
        laneFeature = laneFeature.cpu().numpy() # gai
        batch_size = seg.size(0)
        if batch_size < 240: # 240 90
            return
        laneFeature = laneFeature.reshape(batch_size, -1)
        ipca.partial_fit(laneFeature)




    def pcaDataSet(self, output, batch, ipca, folderName):
        seg, laneFeature = output['seg'], output['lane_feature'] # gai
        laneFeature = laneFeature.cpu().numpy() # gai
        batch_size = seg.size(0)
        laneFeature = laneFeature.reshape(batch_size, -1)
        img_name = batch['meta']['img_name']
        for i in range(batch_size):
            transformedFeature = ipca.transform(laneFeature[i])
            featureOutdir = os.path.join(self.cfg.work_dir, folderName)
            featureOutname = featureOutdir + img_name[i][:-4] + '.txt'
            featureoutdir = os.path.dirname(featureOutname)
            if not os.path.exists(featureoutdir):
                os.makedirs(featureoutdir)
            np.savetxt(featureOutname, transformedFeature) # gai
