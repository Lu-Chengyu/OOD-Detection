...
from .enhancemodel import enhance_net, enhance_net_nopool
...
class RESANet(nn.Module):
    def __init__(self, cfg):
        ...
        self.cfg = cfg
        self.enhance = enhance_net()  # gai
        self.backbone = ResNetWrapper(cfg)
        self.resa = RESA(cfg)
        self.decoder = eval(cfg.decoder)(cfg)
        self.heads = ExistHead(cfg) 

    def forward(self, batch, mean):
        enhanced_image, A, batchz= self.enhance(batch+torch.cat([mean.repeat(batch.size()[0], 1, 1, 1) * 103.939, mean.repeat(batch.size()[0], 1, 1, 1) * 116.779,
                   mean.repeat(batch.size()[0], 1, 1, 1) * 123.68], 1))  # gai
        fea = self.backbone(batchz-torch.cat([mean.repeat(batchz.size()[0], 1, 1, 1) * 103.939, mean.repeat(batchz.size()[0], 1, 1, 1) * 116.779,
                   mean.repeat(batchz.size()[0], 1, 1, 1) * 123.68], 1))  # gai
        fea = self.resa(fea)
        seg = self.decoder(fea)
        # seg, laneFeature = self.decoder(fea)
        exist = self.heads(fea)

        output = {'seg': seg, 'exist': exist, 'enhanced_image': enhanced_image, 'A': A, 'img_lowlight': (batch+torch.cat([mean.repeat(batch.size()[0], 1, 1, 1) * 103.939, mean.repeat(batch.size()[0], 1, 1, 1) * 116.779,
                   mean.repeat(batch.size()[0], 1, 1, 1) * 123.68], 1))/255.0, 'lane_feature':fea}
